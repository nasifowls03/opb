...
export async function execute(interactionOrMessage, client, currentPage = 1) {
  ...
  let eastBlueTotal = 0;
  let eastBlueMax = 0;
  const eastBlueIslands = [];
  for (const isl of islands) {
    if (isl.excludeFromEastBlue) continue;
    eastBlueIslands.push(isl);
  ...
  // Split East Blue islands into 2 pages
  const midPoint = Math.ceil(eastBlueIslands.length / 2);
  const page1Islands = eastBlueIslands.slice(0, midPoint);
  const page2Islands = eastBlueIslands.slice(midPoint);
  const totalPages = 2;

  const getCurrentPageIslands = () => currentPage === 1 ? page1Islands : page2Islands;
  const pageIslands = getCurrentPageIslands();

  const fields = [];
  
  // Show East Blue summary on both pages
  ...
  const nav = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId(`map_nav:back:${userId}`).setLabel('Back').setStyle(ButtonStyle.Secondary).setDisabled(currentPage === 1),
    new ButtonBuilder().setCustomId(`map_nav:next:${userId}`).setLabel('Next').setStyle(ButtonStyle.Secondary).setDisabled(currentPage === totalPages)
  );
