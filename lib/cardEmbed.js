...
export function buildUserCardEmbed(card, ownedEntry, viewer, equippedWeapon = null, teamBanner = null) {
  ...
  // Apply team boosts if they exist in ownedEntry
  let teamAtkBoost = 0, teamHpBoost = 0;
  if (ownedEntry && ownedEntry.boost) {
    const b = ownedEntry.boost;
    if (b.atk) teamAtkBoost = Math.round(userAttackMin * (b.atk / 100));
    if (b.hp) teamHpBoost = Math.round(userHealth * (b.hp / 100));
  }

  // Apply banner boost if equipped
  const bannerSignature = ['Alvida_c_01', 'heppoko_c_01', 'Peppoko_c_01', 'Poppoko_c_01', 'koby_c_01'];
  let bannerAtkBoost = 0, bannerHpBoost = 0;
  let showBannerBoost = false;
  if (teamBanner && teamBanner === 'alvida_pirates_banner_c_01' && bannerSignature.includes(card.id)) {
    bannerAtkBoost = Math.round((userAttackMin + teamAtkBoost) * 0.05);
    bannerHpBoost = Math.round((userHealth + teamHpBoost) * 0.05);
    showBannerBoost = true;
  }

  const finalPower = roundNearestFive(Math.round(userPower + teamAtkBoost));
  const finalAttackMin = Math.round(userAttackMin + teamAtkBoost + bannerAtkBoost);
  const finalAttackMax = Math.round(userAttackMax + teamAtkBoost + bannerAtkBoost);
  const finalHealth = roundNearestFive(Math.round(userHealth + teamHpBoost + bannerHpBoost));
  ...
  if (finalPower > 0) {
    statsParts.push(`**Power:** ${finalPower}`);
  }
  if (finalHealth > 0) {
    let healthText = `**Health:** ${finalHealth}`;
    if (hpBoost > 0) healthText += ` (includes +${hpBoost} from weapon)`;
    if (showBannerBoost && bannerHpBoost > 0) healthText += ` (+${bannerHpBoost} boosted from banner)`;
    statsParts.push(healthText);
  }

  if (finalAttackMin > 0 || finalAttackMax > 0) {
    let attackText = `**Attack:** ${finalAttackMin} - ${finalAttackMax}`;
    if (showBannerBoost && bannerAtkBoost > 0) attackText += ` (+${bannerAtkBoost} boosted from banner)`;
