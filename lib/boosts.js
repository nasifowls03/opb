...
export function computeTeamBoosts(teamIds, userCardsMap = null, weaponInventory = null) {
  ...
  for (const id of teamIds) {
    ...
    // Check for banner boost first
    if (weaponInventory && weaponInventory.teamBanner) {
      const bannerCard = getCardById(weaponInventory.teamBanner);
      if (bannerCard && bannerCard.boost && bannerCard.signatureCards && bannerCard.signatureCards.includes(id)) {
        if (bannerCard.boost.atk) out.atk += bannerCard.boost.atk;
        if (bannerCard.boost.hp) out.hp += bannerCard.boost.hp;
        if (bannerCard.boost.special) out.special += bannerCard.boost.special;
      }
    }
...
export function computeTeamBoostsDetailed(teamIds, userCardsMap = null, weaponInventory = null) {
  ...
  for (const id of teamIds) {
    ...
    if (weaponInventory && weaponInventory.teamBanner) {
      const bannerCard = getCardById(weaponInventory.teamBanner);
      if (bannerCard && bannerCard.boost && bannerCard.signatureCards && bannerCard.signatureCards.includes(id)) {
        entry.atk += bannerCard.boost.atk || 0;
        entry.hp += bannerCard.boost.hp || 0;
        entry.special += bannerCard.boost.special || 0;
        entry.reason = entry.reason ? entry.reason + ', banner boost' : 'banner boost';
      }
    }
